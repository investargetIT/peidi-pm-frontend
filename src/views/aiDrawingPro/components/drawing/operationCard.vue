  ...
  async function formatParams() {
    ...
    return {
      ...
      prompt: `
        ...
        6. 文字里的“\n”，表示换行，不要保留在文字里。
        7. 文字“${item.productName}”背后的绿色必须是纯绿色，不要有任何其他颜色。
        ...
        ────────────────────
        【排版避让与文字边界规则 · 核心修正（针对卖点文案）】
        ⚠️ 针对顶部产品卖点文案（如“特色酥骨工艺 蜜汁兔脊”）：

        【虚拟边界设定】
          - 虽然右上角“店铺 LOGO”已被视觉抹除，但**必须保留其“空间占位”**。
          - 文案排版的右侧边界，必须严格限制在原 LOGO 左边缘以左。

        【长文案处理逻辑】
          - 当卖点文案过长，即将触及右上角预留区域时，**必须**触发以下机制之一（按优先级）：
            a. **缩小字号**：保持单行，但按比例缩小字体，确保不越界。
            b. **自动换行**：将文案折行为两行，保持左对齐或居中对齐（跟随原模版对齐方式）。

        【失败判定】
          - 任何文字像素延伸至右上角原圆盘LOGO区域内，均视为严重错误（会导致后续贴图遮挡）。

        ────────────────────
        【中间左侧产品标题“${item.productName}”背景色块 · 强保护规则】
