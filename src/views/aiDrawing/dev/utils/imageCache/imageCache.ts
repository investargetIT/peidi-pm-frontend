...
class ImageCache {
   ...
   * 更新图片的压缩数据
   * @param id 图片ID
   * @param compressedBlob 压缩后的base64数据
   * @returns Promise<boolean> 更新是否成功
   */
  public async updateCompressedBlob(
    id: string,
    compressedBlob: string
  ): Promise<boolean> {
    try {
      ...
      return await this.storeImage(
        id,
        existingData.originalBlob,
        compressedBlob
      );
    } catch (error) {
      console.error("更新压缩数据失败:", error);
      return false;
    }
  }
   ...
   * 更新图片的源文件数据
   * @param id 图片ID
   * @param originalBlob 源文件base64数据
   * @returns Promise<boolean> 更新是否成功
   */
  public async updateOriginalBlob(
    id: string,
    originalBlob: string
  ): Promise<boolean> {
    try {
      ...
      return await this.storeImage(
        id,
        originalBlob,
        existingData.compressedBlob
      );
    } catch (error) {
      console.error("更新源文件数据失败:", error);
      return false;
    }
  }
  ...
  public async getAllImageIds(): Promise<string[]> {
    try {
      ...
      console.error("获取图片ID列表失败:", error);
      return [];
    }
  }
