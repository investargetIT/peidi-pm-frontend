...
const handleSearchParams = () => {
  ...
  if (searchForm.value.examinationType) {
    searchStr.push({
      searchName: "examinationType",
      searchType: "like",
      searchValue: searchForm.value.examinationType
    });
  }
  if (searchForm.value.department1) {
    searchStr.push({
      searchName: "department1",
      searchType: "like",
      searchValue: searchForm.value.department1
    });
  }
  if (searchForm.value.department2) {
    searchStr.push({
      searchName: "department2",
      searchType: "like",
      searchValue: searchForm.value.department2
    });
  }
  ...
  getExaminationRecordResult({
    ...
    .then((res: any) => {
      if (res.code === 200) {
        ...
        monthList.value = res.data[0].examination[0].data
        ...
        res.data.forEach((item: any) => {
          item.examination.forEach((examinationItem: any) => {
            const monthData = examinationItem.data.reduce(
              (acc: any, cur: any) => {
...
const last3MonthList = computed(() => {
  return monthList.value.length >= 3
    ? monthList.value.slice(-3)
    : monthList.value;
});
  ...
  <div>
    <el-card shadow="never">
      <el-form :model="searchForm" inline>
        ...
        <el-form-item label="指标名称" prop="examinationType">
          <el-input
            v-model="searchForm.examinationType"
            placeholder="请输入指标名称"
            clearable
            style="width: 200px"
          />
        ...
        <el-form-item label="一级部门" prop="department1">
          <el-input
            v-model="searchForm.department1"
            placeholder="请输入一级部门"
            clearable
            style="width: 200px"
          />
        ...
        <el-form-item label="二级部门" prop="department2">
          <el-input
            v-model="searchForm.department2"
            placeholder="请输入二级部门"
            clearable
            style="width: 200px"
          />
